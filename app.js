...
async function pushClase(dia,editId="null"){
    ...
    async function recuperarSemanaEliminada(){
      ...
      if(existentes.length){
        const reemplazar=confirm('Ya hay clases en esta semana. Si recuperas, se reemplazaran por la version eliminada. øDeseas continuar?');
    ...
    function markSolicitudesAsSeen(){
      ...
      if(!CACHE_SOLICITUDES.length){
        cont.innerHTML='<p style="text-align:center;opacity:.3;font-size:.7rem;font-style:italic;margin-top:30px">No hay solicitudes a√∫n.</p>';
        return;
      }
...
function renderCronograma(){
    ...
    function processVencimientos(){
      ...
      const htmlCumple=cumplePend.map(c=>{
        ...
        return `<div class="clase-box birthday-inline" style="border-left:4px solid #ffd36a;display:flex;justify-content:space-between;align-items:center;gap:12px"><div><b style="color:#ffd36a">Nuevo cumpleaÒos</b><br><small>${c.nombre} esta cumpliendo anos</small></div>${ws}</div>`;
      }).join('');
      ...
      const empty=(alertas.length===0&&cumplePend.length===0)
        ? '<p style="text-align:center;opacity:.3;font-size:.7rem;font-style:italic;margin-top:30px">No hay vencimientos ni cumpleaÒos hoy.</p>'
        : '';
    ...
    function buildBirthdayWhatsappMessage(nombre){
      ...
      return 'Hola '+(nombre||'')+'. Pilates Pulse te desea un feliz cumpleaÒos, gracias por compartir tu energÌa y tu esfuerzo con nosotros. Que este aÒo sigas creciendo con fluidez y control'+wave +  meditate+'\nPilates Pulse.';
    }
    ...
    function openBirthdayPrompt(cumple){
      ...
      +     '<div class="birthday-prompt-title">Nuevo cumpleaÒos</div>'
    ...
    function abrirFormNuevoAlumno(editId=null,existingData=null){
      ...
      const objetivoOps=['Mejorar postura','Aumentar flexibilidad','Fortalecer el cuerpo','Disminuir dolor','Recuperaci√≥n de lesi√≥n','Bienestar','Otro'];
...
function buildMiniCalendar(dateObj){
  ...
  return `
    ...
    <div class="mini-cal-details">
      ...
      <div class="mini-cal-daynote">${todayInfo.hasAgendaDay ? `D√≠a activo: ${todayInfo.dia}` : 'Hoy no hay agenda (domingo)'}</div>
    </div>`;
}
