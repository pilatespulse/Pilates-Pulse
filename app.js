...
function processVencimientos(){
    ...
    function abrirFormNuevoAlumno(editId=null,existingData=null){
      ...
      const formHtml=`<div class="modal-form-shell">
        <div style="width:100%;box-sizing:border-box;overflow:hidden"><label>Nombre completo</label><input type="text" id="db-nom" value="${nombre}" style="width:100%;box-sizing:border-box"></div>
        <div style="width:100%;box-sizing:border-box;overflow:hidden"><label>Fecha de nacimiento</label><input type="date" id="db-nac" value="${nac}" style="width:100%;box-sizing:border-box;min-width:0;display:block"></div>
        <div style="width:100%;box-sizing:border-box;overflow:hidden"><label>Tel&eacute;fono</label><input type="text" id="db-tel" value="${tel}" style="width:100%;box-sizing:border-box"></div>
        <div style="width:100%;box-sizing:border-box;overflow:hidden"><label>Profesi&oacute;n / actividad principal</label><input type="text" id="db-prof" value="${profesion}" style="width:100%;box-sizing:border-box"></div>
        <div style="width:100%;box-sizing:border-box;overflow:hidden"><label>Vencimiento</label><input type="date" id="db-venc" value="${venc}" style="width:100%;box-sizing:border-box;min-width:0;display:block"></div>

        <div style="width:100%;box-sizing:border-box;overflow:hidden"><label>&iquest;Realiza actualmente alguna actividad f&iacute;sica?</label>
        <select id="db-activa" onchange="toggleConditionalInput('db-activa','db-activa-cual-wrap')" style="width:100%;box-sizing:border-box">
          <option ${act==='No'?'selected':''}>No</option>
          <option ${act==='Si'?'selected':''}>Si</option>
        </select></div>
        <div id="db-activa-cual-wrap" style="display:none">
          <div style="width:100%;box-sizing:border-box;overflow:hidden"><label>&iquest;Cu&aacute;l?</label><input type="text" id="db-activa-cual" value="${actCual}" style="width:100%;box-sizing:border-box"></div>
        </div>

        <div style="width:100%;box-sizing:border-box;overflow:hidden"><label>&iquest;Tiene actualmente dolor o alguna lesi&oacute;n?</label>
        <select id="db-dolor" onchange="toggleConditionalInput('db-dolor','db-dolor-donde-wrap')" style="width:100%;box-sizing:border-box">
          <option ${dolor==='No'?'selected':''}>No</option>
          <option ${dolor==='Si'?'selected':''}>Si</option>
        </select></div>
        <div id="db-dolor-donde-wrap" style="display:none">
          <div style="width:100%;box-sizing:border-box;overflow:hidden"><label>&iquest;D&oacute;nde?</label><input type="text" id="db-dolor-donde" value="${dolorDonde}" style="width:100%;box-sizing:border-box"></div>
        </div>

        <div style="width:100%;box-sizing:border-box;overflow:hidden"><label>&iquest;Ha tenido alguna cirug&iacute;a importante?</label>
        <select id="db-cirugia" onchange="toggleConditionalInput('db-cirugia','db-cirugia-cual-wrap')" style="width:100%;box-sizing:border-box">
          <option ${cirugia==='No'?'selected':''}>No</option>
          <option ${cirugia==='Si'?'selected':''}>Si</option>
        </select></div>
        <div id="db-cirugia-cual-wrap" style="display:none">
          <div style="width:100%;box-sizing:border-box;overflow:hidden"><label>&iquest;Cu&aacute;l?</label><input type="text" id="db-cirugia-cual" value="${cirugiaCual}" style="width:100%;box-sizing:border-box"></div>
        </div>

        <div style="width:100%;box-sizing:border-box;overflow:hidden"><label>&iquest;En qu&eacute; parte del cuerpo siente m&aacute;s tensi&oacute;n o molestias habitualmente?</label>
        <select id="db-tension" onchange="toggleConditionalInput('db-tension','db-tension-otro-wrap','Otro')" style="width:100%;box-sizing:border-box">${tensionOps.map(o=>`<option ${tension===o?'selected':''}>${o}</option>`).join('')}</select></div>
        <div id="db-tension-otro-wrap" style="display:none">
          <div style="width:100%;box-sizing:border-box;overflow:hidden"><label>Otro (especifique)</label><input type="text" id="db-tension-otro" value="${tensionOtro}" style="width:100%;box-sizing:border-box"></div>
        </div>
        ...
        <div id="db-info-extra" style="display:none;margin-top:8px">
          <div style="width:100%;box-sizing:border-box;overflow:hidden"><label>&iquest;Est&aacute; embarazada?</label>
          <select id="db-embarazo" style="width:100%;box-sizing:border-box">
            <option ${embarazo==='No'?'selected':''}>No</option>
            <option ${embarazo==='Si'?'selected':''}>Si</option>
          </select></div>

          <div style="width:100%;box-sizing:border-box;overflow:hidden"><label>&iquest;Ha tenido partos o ces&aacute;reas?</label>
          <select id="db-partos" onchange="toggleConditionalInput('db-partos','db-partos-cuantos-wrap')" style="width:100%;box-sizing:border-box">
            <option ${partos==='No'?'selected':''}>No</option>
            <option ${partos==='Si'?'selected':''}>Si</option>
          </select></div>
          <div id="db-partos-cuantos-wrap" style="display:none">
            <div style="width:100%;box-sizing:border-box;overflow:hidden"><label>&iquest;Cu&aacute;ntos?</label><select id="db-partos-cuantos" style="width:100%;box-sizing:border-box">${nums}</select></div>
          </div>

          <div style="width:100%;box-sizing:border-box;overflow:hidden"><label>Objetivo principal al venir al estudio</label>
          <select id="db-objetivo" onchange="toggleConditionalInput('db-objetivo','db-objetivo-otro-wrap','Otro')" style="width:100%;box-sizing:border-box">${objetivoOps.map(o=>`<option ${objetivo===o?'selected':''}>${o}</option>`).join('')}</select></div>
          <div id="db-objetivo-otro-wrap" style="display:none">
            <div style="width:100%;box-sizing:border-box;overflow:hidden"><label>Otro (especifique)</label><input type="text" id="db-objetivo-otro" value="${objetivoOtro}" style="width:100%;box-sizing:border-box"></div>
          </div>

          <div style="width:100%;box-sizing:border-box;overflow:hidden"><label>Observaciones</label><textarea id="db-obs" rows="3" style="width:100%;box-sizing:border-box">${obs}</textarea></div>
        </div>
        ...
        <div style="display:grid; grid-template-columns:1fr; gap:12px; background:rgba(255,255,255,0.02); border:1px solid rgba(255,255,255,0.08); padding:14px; border-radius:16px; margin-bottom:15px;">
          <div>
            ...
            ` : ''}
            <input type="file" id="db-foto-1" accept="image/*" style="display:none" onchange="updateFileLabel('db-foto-1', 'db-foto-1-label')">
            <button type="button" id="db-foto-1-label" class="btn-second" style="width:100%; font-size:0.65rem; padding:10px; margin-bottom:8px;" onclick="document.getElementById('db-foto-1').click()">Subir foto</button>
          </div>
          <div>
            ...
            ` : ''}
            <input type="file" id="db-foto-2" accept="image/*" style="display:none" onchange="updateFileLabel('db-foto-2', 'db-foto-2-label')">
            <button type="button" id="db-foto-2-label" class="btn-second" style="width:100%; font-size:0.65rem; padding:10px; margin-bottom:8px;" onclick="document.getElementById('db-foto-2').click()">Subir foto</button>
          </div>
          <div>
            ...
            ` : ''}
            <input type="file" id="db-foto-3" accept="image/*" style="display:none" onchange="updateFileLabel('db-foto-3', 'db-foto-3-label')">
            <button type="button" id="db-foto-3-label" class="btn-second" style="width:100%; font-size:0.65rem; padding:10px; margin-bottom:8px;" onclick="document.getElementById('db-foto-3').click()">Subir foto</button>
          </div>
        </div>
    ...
    function updateFileLabel(inputId, buttonId){
      const input = document.getElementById(inputId);
      const button = document.getElementById(buttonId);
      if(input && button && input.files && input.files[0]){
        button.textContent = input.files[0].name;
        button.style.background = 'rgba(182, 133, 95, 0.2)';
        button.style.borderColor = 'rgba(182, 133, 95, 0.4)';
      }
    }
  ...
  const bodyHtml = `
    ...
    <div class="interactive-tab-content active-content" id="tab-content-history">
      <div class="history-grid">
        ...
        <button class="btn-principal" style="margin-top: 16px; font-size: .65rem; padding: 14px; letter-spacing: 1.5px;" onclick="openEditMedicalHistory('${id}')">Editar Historial Médico</button>
...
function openEditMedicalHistory(studentId) {
  const student = CACHE_ALUMNOS.find(a => String(a.id) === String(studentId));
  if (!student) {
    alert("No se encontró la información de la alumna.");
    return;
  }

  const p = student.contenido.split('|');
  const get = (idx, def = '') => typeof p[idx] !== 'undefined' ? p[idx] : def;
  ...
  const formHtml = `
    <div class="modal-form-shell">
      <label>¿Realiza actualmente alguna actividad física?</label>
      <select id="edit-activa" onchange="toggleConditionalInput('edit-activa','edit-activa-cual-wrap')">
        <option ${act === 'No' ? 'selected' : ''}>No</option>
        <option ${act === 'Si' ? 'selected' : ''}>Si</option>
      </select>
      <div id="edit-activa-cual-wrap" style="display:${act === 'Si' ? 'block' : 'none'}">
        <label>¿Cuál?</label><input type="text" id="edit-activa-cual" value="${actCual}">
      </div>

      <label>¿Tiene actualmente dolor o alguna lesión?</label>
      <select id="edit-dolor" onchange="toggleConditionalInput('edit-dolor','edit-dolor-donde-wrap')">
        <option ${dolor === 'No' ? 'selected' : ''}>No</option>
        <option ${dolor === 'Si' ? 'selected' : ''}>Si</option>
      </select>
      <div id="edit-dolor-donde-wrap" style="display:${dolor === 'Si' ? 'block' : 'none'}">
        <label>¿Dónde?</label><input type="text" id="edit-dolor-donde" value="${dolorDonde}">
      </div>

      <label>¿Ha tenido alguna cirugía importante?</label>
      <select id="edit-cirugia" onchange="toggleConditionalInput('edit-cirugia','edit-cirugia-cual-wrap')">
        <option ${cirugia === 'No' ? 'selected' : ''}>No</option>
        <option ${cirugia === 'Si' ? 'selected' : ''}>Si</option>
      </select>
      <div id="edit-cirugia-cual-wrap" style="display:${cirugia === 'Si' ? 'block' : 'none'}">
        <label>¿Cuál?</label><input type="text" id="edit-cirugia-cual" value="${cirugiaCual}">
      </div>

      <label>¿En qué parte del cuerpo siente más tensión o molestias habitualmente?</label>
      <select id="edit-tension" onchange="toggleConditionalInput('edit-tension','edit-tension-otro-wrap','Otro')">${tensionOps.map(o => `<option ${tension === o ? 'selected' : ''}>${o}</option>`).join('')}</select>
      <div id="edit-tension-otro-wrap" style="display:${tension === 'Otro' ? 'block' : 'none'}">
        <label>Otro (especifique)</label><input type="text" id="edit-tension-otro" value="${tensionOtro}">
      </div>

      <label>¿Está embarazada?</label>
      <select id="edit-embarazo">
        <option ${embarazo === 'No' ? 'selected' : ''}>No</option>
        <option ${embarazo === 'Si' ? 'selected' : ''}>Si</option>
      </select>

      <label>¿Ha tenido partos o cesáreas?</label>
      <select id="edit-partos" onchange="toggleConditionalInput('edit-partos','edit-partos-cuantos-wrap')">
        <option ${partos === 'No' ? 'selected' : ''}>No</option>
        <option ${partos === 'Si' ? 'selected' : ''}>Si</option>
      </select>
      <div id="edit-partos-cuantos-wrap" style="display:${partos === 'Si' ? 'block' : 'none'}">
        <label>¿Cuántos?</label><select id="edit-partos-cuantos">${nums}</select>
      </div>

      <label>Objetivo principal al venir al estudio</label>
      <select id="edit-objetivo" onchange="toggleConditionalInput('edit-objetivo','edit-objetivo-otro-wrap','Otro')">${objetivoOps.map(o => `<option ${objetivo === o ? 'selected' : ''}>${o}</option>`).join('')}</select>
      <div id="edit-objetivo-otro-wrap" style="display:${objetivo === 'Otro' ? 'block' : 'none'}">
        <label>Otro (especifique)</label><input type="text" id="edit-objetivo-otro" value="${objetivoOtro}">
      </div>

      <label>Observaciones</label><textarea id="edit-obs" rows="3">${obs}</textarea>

      <label style="color:#ffd36a; font-weight:800; margin-top:20px; margin-bottom:10px; border-bottom:1px solid rgba(255,255,255,0.08); padding-bottom:5px; font-size:0.6rem; letter-spacing:1px;">Autorización de Participación</label>
      <label>¿Autorizó clases bajo su responsabilidad?</label>
      <select id="edit-autorizo">
        <option ${!aut ? 'selected' : ''}>No</option>
        <option ${aut ? 'selected' : ''}>Si</option>
      </select>

      <button class="btn-principal" style="margin-top:20px; padding:14px; font-size:.65rem; letter-spacing:1.5px;" onclick="saveMedicalHistory('${studentId}')">Guardar Cambios</button>
      <button class="btn-cancelar" style="padding:12px; font-size:.6rem;" onclick="closeModal()">Cancelar</button>
    </div>
  `;

  openModal('Editar Historial Médico', formHtml);
}

async function saveMedicalHistory(studentId) {
  const student = CACHE_ALUMNOS.find(a => String(a.id) === String(studentId));
  if (!student) {
    alert('No se encontró la información de la alumna.');
    return;
  ...
  const activa = document.getElementById('edit-activa').value;
  const activaCual = document.getElementById('edit-activa-cual').value;
  const dolor = document.getElementById('edit-dolor').value;
  const dolorDonde = document.getElementById('edit-dolor-donde').value;
  const cirugia = document.getElementById('edit-cirugia').value;
  const cirugiaCual = document.getElementById('edit-cirugia-cual').value;
  const tension = document.getElementById('edit-tension').value;
  const tensionOtro = document.getElementById('edit-tension-otro').value;
  const embarazo = document.getElementById('edit-embarazo').value;
  const partos = document.getElementById('edit-partos').value;
  const partosCuantos = document.getElementById('edit-partos-cuantos').value;
  const objetivo = document.getElementById('edit-objetivo').value;
  const objetivoOtro = document.getElementById('edit-objetivo-otro').value;
  const obs = document.getElementById('edit-obs').value;
  const autorizo = document.getElementById('edit-autorizo').value;

  const safe = (v) => (v || '').trim().replace(/[<>`]/g, '');

  p[14] = safe(activa);
  p[15] = safe(activaCual);
  p[16] = safe(dolor);
  p[17] = safe(dolorDonde);
  p[18] = safe(cirugia);
  p[19] = safe(cirugiaCual);
  p[20] = safe(tension === 'Otro' ? tensionOtro : tension);
  p[21] = safe(embarazo);
  p[22] = safe(partos);
  p[23] = safe(partosCuantos);
  p[24] = safe(objetivo === 'Otro' ? objetivoOtro : objetivo);
  p[25] = safe(obs);
  p[26] = safe(autorizo);

  const content = p.join('|');

  const { error } = await _sp.from('horarios').update({ contenido: content }).eq('id', studentId);
  ...
  const updatedStudent = CACHE_ALUMNOS.find(a => String(a.id) === String(studentId));
  if (updatedStudent) {
    openStudentInteractivePanel(studentId, updatedStudent.contenido.split('|')[1]);
  }
}
